...
>(({
  ...
  const categorizedIcons = useMemo(() => {
    if (!categorized || search.trim() !== "") {
      return [{ name: allIconsLabel, icons: filteredIcons }];
    }
        ...
        {children || (
          <button
            ...
            className={cn(
              ...
              (value || selectedIcon) ? "w-auto gap-2" : "w-full",
              !(value || selectedIcon) && "text-muted-foreground"
            )}
          >
            <span className="flex items-center gap-2 truncate">
              {(value || selectedIcon) ? (
                <Icon name={(value || selectedIcon)!} className="h-4 w-4" />
              ) : (
                triggerPlaceholder
              )}
            </span>
            <DynamicIcon name="chevron-down" className="h-4 w-4 opacity-50 shrink-0" />
          </button>
        )}
      </PopoverTrigger>
        ...
        {categorized && search.trim() === "" && (
          <div className="relative mt-2 pb-2">
            <div className="flex flex-row items-center gap-1">
              ...
              <div 
                className="flex flex-row gap-1 overflow-x-auto scrollbar-none flex-1"
