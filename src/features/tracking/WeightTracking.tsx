    ...
    return (
        <div className="space-y-5 overflow-hidden w-full">
            ...
            <div className="space-y-1.5">
                <h3 className="text-md font-medium">Résumé</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    ...
                    <div className={`p-3 rounded-md border min-w-0 ${isTargetReached ? 'bg-green-500/10 border-green-500/20' : 'bg-orange-500/10 border-orange-500/20'}`}>
                        <p className={`text-sm leading-tight ${isTargetReached ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>Dernière mesure</p>
            ...
            <div className="space-y-2 border-t pt-2">
                <div className="flex items-center justify-between">
                    <h3 className="text-md font-medium">Pesées</h3>
                    <Button
                        type="button"
                        variant="outline"
                        size="sm"
                ...
                {initialWeight > 0 && (
                    <div className="flex items-center gap-3 bg-muted/50 p-2 rounded-md">
                        <span className="flex-1 text-muted-foreground text-sm min-w-0">
                            {format(new Date(item.created_at), "d MMMM yyyy 'à' HH:mm", { locale: fr })} (pesée initiale)
                        </span>
                    ...
                    <div className="space-y-2">
                        {logs.map((log) => (
                            <div key={log.id} className="flex items-center gap-3 bg-muted/50 p-2 rounded-md">
                                <span className="flex-1 text-muted-foreground text-sm min-w-0">
                                    {format(new Date(log.date), "d MMMM yyyy 'à' HH:mm", { locale: fr })}
                                </span>
            ...
            {(logs.length > 0 || initialWeight > 0) && (
                <div className="space-y-1.5 border-t pt-2">
                    <h3 className="text-md font-medium">Évolution du poids</h3>
                    ...
                    {projectedEndDate && !isTargetReached && (
                        <div className="bg-orange-500/10 border border-orange-500/20 p-1.5 rounded-md text-center">
                            <p className="text-sm leading-tight">
                                <span className="text-muted-foreground">Fin d'affinage prévue : </span>
